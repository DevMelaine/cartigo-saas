...
export function ActivityHeatmap({ data }: ActivityHeatmapProps) {
  const chartData = data.map((day) => ({
    day: day.day,
    activity: day.hours.reduce((total, value) => total + value, 0),
  }))

  return (
    <Card className="col-span-3">
      <CardHeader>
        <CardTitle>Activite par jour</CardTitle>
      </CardHeader>
      <CardContent>
        <ResponsiveContainer width="100%" height={350}>
          <BarChart data={chartData}>
            ...
            />
            <Bar dataKey="activity" fill={dashboardChartPalette[2]} radius={[10, 10, 0, 0]} />
          </BarChart>
        </ResponsiveContainer>
      </CardContent>
    </Card>
